...
if exists("c_fold_blocks")
    ...
    if exists("c_curly_error")
      syntax match cCurlyError "}"
      syntax region	cBlock		start="\z(\s*\)\(if\|else\|while\|for\|do\|switch\)\@<!\s*(.*)\s*{" end="\z1}" contains=ALLBUT,cCurlyError,@cParenGroup,cErrInParen,cCppParen,cErrInBracket,cCppBracket,cCppString,@Spell fold keepend
      syntax region	cBlock		start="\z(\s*\)\(union\|enum\|struct\)\s\+(\I\i*)\?\s*{" end="\z1}" contains=ALLBUT,cCurlyError,@cParenGroup,cErrInParen,cCppParen,cErrInBracket,cCppBracket,cCppString,@Spell fold keepend
    else
      syntax region	cBlock		start="\z(\s*\)\(if\|else\|while\|for\|do\|switch\)\@<!\s*(.*)\s*{" end="\z1}" transparent fold keepend
      syntax region	cBlock		start="\z(\s*\)\(union\|enum\|struct\)\s\+(\I\i*)\?\s*{" end="\z1}" transparent fold keepend
    endif
endif
